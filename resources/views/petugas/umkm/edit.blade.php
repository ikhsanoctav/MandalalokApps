...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-slate-800">Edit Data UMKM</h2>
            <a href="{{ route('operator.dashboard') }}" class="text-sm text-blue-600 hover:underline flex items-center gap-1">
        ...
        <form action="{{ route('operator.umkm.update', $umkm->id_umkm) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="nik" value="{{ old('nik', $umkm->pemilik->nik ?? '') }}" required maxlength="16"
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $umkm->pemilik->nama_lengkap ?? '') }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $umkm->pemilik->tempat_lahir ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="email" name="email_pemilik" value="{{ old('email_pemilik', $umkm->pemilik->email ?? '') }}"
                    ...
                    <div x-data="umkmWilayahDropdown()"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="alamat_pemilik" required rows="2" placeholder="Nama Jalan, Nomor Rumah, RT/RW, dsb"
                            class="w-full px-4 py-2.5 rounded-lg border border-slate-300 bg-white text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200">{{ old('alamat_pemilik', $umkm->pemilik->alamat ?? '') }}</textarea>
                ...
                <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="id_kategori" required
                            ...
                            @foreach ($kategoris ?? $kategori as $kat)
                                <option value="{{ $kat->id }}"
                                    {{ old('id_kategori', $umkm->id_kategori ?? '') == $kat->id ? 'selected' : '' }}>{{ $kat->nama_kategori }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="id_sektor" required
                            ...
                            @foreach ($sektors ?? $sektor as $sek)
                                <option value="{{ $sek->id }}" {{ old('id_sektor', $umkm->id_sektor ?? '') == $sek->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="status_usaha"
                            class="w-full px-4 py-2.5 rounded-lg border border-slate-300 bg-white text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200">
                            <option value="aktif" {{ old('status_usaha', $umkm->status_usaha ?? '') == 'aktif' ? 'selected' : '' }}>Aktif
                            </option>
                            <option value="non_aktif" {{ old('status_usaha', $umkm->status_usaha ?? '') == 'non_aktif' ? 'selected' : '' }}>Non Aktif
                            </option>
                    ...
                    <div>
                        ...
                        <input type="text" name="no_izin_usaha" value="{{ old('no_izin_usaha', $umkm->no_izin_usaha ?? '') }}"
                    ...
                    <div>
                        ...
                        <select name="jenis_izin"
                            ...
                            <option value="Lainnya" {{ old('jenis_izin', $umkm->jenis_izin ?? '') == 'Lainnya' ? 'selected' : '' }}>Lainnya
                            </option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="npwp_usaha" value="{{ old('npwp_usaha', $umkm->npwp_usaha ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="telp_usaha" value="{{ old('telp_usaha', $umkm->telp_usaha ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="email" name="email_usaha" value="{{ old('email_usaha', $umkm->email_usaha ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="website" value="{{ old('website', $umkm->website ?? '') }}"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="alamat_usaha" required rows="2" placeholder="Nama Jalan, RT/RW, Patokan"
                            class="w-full px-4 py-2.5 rounded-lg border border-slate-300 bg-white text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200">{{ old('alamat_usaha', $umkm->alamat_usaha ?? '') }}</textarea>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" id="latitude" name="latitude" value="{{ old('latitude', $umkm->latitude ?? '') }}" readonly class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-slate-50 text-slate-600 text-sm focus:outline-none">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-slate-600 mb-1">Longitude</label>
                                <input type="text" id="longitude" name="longitude" value="{{ old('longitude', $umkm->longitude ?? '') }}" readonly class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-slate-50 text-slate-600 text-sm focus:outline-none">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="deskripsi" rows="3" placeholder="Jelaskan produk atau jasa yang ditawarkan..."
                            class="w-full px-4 py-2.5 rounded-lg border border-slate-300 bg-white text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200">{{ old('deskripsi', $umkm->deskripsi ?? '') }}</textarea>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="p-5 grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="number" name="jumlah_tenaga_kerja" value="{{ old('jumlah_tenaga_kerja', $umkm->jumlah_tenaga_kerja ?? 0) }}"
                            min="0"
                            class="w-full px-4 py-2.5 rounded-lg border border-slate-300 bg-white text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200">
                    ...
                    <div>
                        ...
                        <input type="number" name="tenaga_kerja_laki" value="{{ old('tenaga_kerja_laki', $umkm->tenaga_kerja_laki ?? 0) }}"
                            min="0"
                            class="w-full px-4 py-2.5 rounded-lg border border-slate-300 bg-white text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200">
                    ...
                    <div>
                        ...
                        <input type="number" name="tenaga_kerja_perempuan"
                            value="{{ old('tenaga_kerja_perempuan', $umkm->tenaga_kerja_perempuan ?? 0) }}" min="0"
                            class="w-full px-4 py-2.5 rounded-lg border border-slate-300 bg-white text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200">
...
<div class="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="media_sosial[facebook]" value="{{ old('media_sosial.facebook', $mediaSosial['facebook'] ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="media_sosial[instagram]" value="{{ old('media_sosial.instagram', $mediaSosial['instagram'] ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="media_sosial[tiktok]" value="{{ old('media_sosial.tiktok', $mediaSosial['tiktok'] ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="media_sosial[whatsapp]" value="{{ old('media_sosial.whatsapp', $mediaSosial['whatsapp'] ?? '') }}"
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 mb-2">Bisa langsung foto dari kamera HP atau pilih dari galeri.
                            Maks 5MB.</p>
                                                                        ...
                                                                        @if ($umkm->foto_utama)
                            <div class="mb-3" id="fotoUtamaContainer">
                                <p class="text-xs text-slate-500 mb-2">Foto Utama Saat Ini:</p>
                                <div class="relative inline-block group">
                                    <img src="{{ Storage::url($umkm->foto_utama) }}" class="h-32 object-cover rounded-xl border border-slate-200">
                                    <button type="button" onclick="document.getElementById('hapus_foto_utama_input').value='1'; document.getElementById('fotoUtamaContainer').style.display='none';" class="absolute top-2 right-2 bg-white/90 p-1.5 rounded-full shadow-sm border border-slate-200 cursor-pointer flex items-center justify-center hover:bg-red-50 hover:text-red-600 text-slate-400 transition-colors opacity-0 group-hover:opacity-100">
...
<label for="foto_utama"
                            ...
                            <input id="foto_utama" name="foto_utama" type="file" class="sr-only" accept="image/*"
                                capture="environment" @if (! $umkm->foto_utama) required @endif>
                        </label>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 mb-2">Bisa pilih banyak foto sekaligus. Maks 5MB per foto.</p>
                            ...
                            <div class="mb-3">
                                <p class="text-xs text-slate-500 mb-2">Foto Gallery Saat Ini:</p>
                                <div class="flex flex-wrap gap-3">
                                @foreach($galleries as $index => $g)
                                    <div class="relative inline-block group" id="gallery_item_{{ $index }}">
                                        <img src="{{ Storage::url($g) }}" class="h-16 w-16 object-cover rounded-xl border border-slate-200">
                                        <button type="button" onclick="document.getElementById('hapus_gallery_container').insertAdjacentHTML('beforeend', '<input type=\'hidden\' name=\'hapus_foto_gallery[]\' value=\'{{ $index }}\'>'); document.getElementById('gallery_item_{{ $index }}').style.display='none';" class="absolute -top-2 -right-2 bg-white p-1 rounded-full shadow-sm border border-slate-200 cursor-pointer flex items-center justify-center hover:bg-red-50 hover:text-red-600 text-slate-400 transition-colors opacity-0 group-hover:opacity-100">
    ...
    @push('scripts')
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                var initialLat = {{ $umkm->latitude ?? -6.914744 }}; // Default to Bandung or current region
