...
<div class="max-w-5xl mx-auto space-y-8 pb-12">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <a href="{{ route('superadmin.pelatihan.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-white border-2 border-slate-200 hover:border-indigo-300 hover:text-indigo-600 text-slate-700 font-bold rounded-lg transition-all shadow-sm">
    ...
    <div class="bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-xl shadow-slate-200/50 border border-white/80 overflow-hidden relative p-8">
        ...
        <form action="{{ route('superadmin.pelatihan.update', $pelatihan->id) }}" method="POST" enctype="multipart/form-data" class="relative z-10">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <p class="text-[10px] text-slate-700 mt-1">Biarkan kosong jika tidak ingin mengubah banner. Maks 2MB, rasio ideal 16:9 (Landscape).</p>
        ...
        <div class="flex items-center justify-between border-t pt-6 gap-3">
            ...
            <div class="flex gap-2">
                <a href="{{ route('superadmin.pelatihan.index') }}" class="px-5 py-2.5 text-sm font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-all">Batal</a>
    ...
    <form id="delete-form" action="{{ route('superadmin.pelatihan.destroy', $pelatihan->id) }}" method="POST" class="hidden">
        @csrf
        @method('DELETE')
    </form>
</div>
