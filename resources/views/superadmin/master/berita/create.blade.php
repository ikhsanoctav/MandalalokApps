...
@section('breadcrumb', 'Master Data / Manajemen Warta / Tambah')
...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-2xl font-bold text-slate-800">Tambah Warta Baru</h2>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Konten Berita <span
                                class="text-red-500">*</span></label>
                        <input id="x" type="hidden" name="konten" value="{{ old('konten') }}"><trix-editor
                            input="x" class="trix-content bg-white min-h-[200px]"></trix-editor>
                        @error('konten')
                            <span class="text-xs text-red-500 mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="file" name="gambar"
                            ...
                            accept="image/*">
                        <p class="text-xs text-slate-700 mt-2">Maksimal 2MB. Boleh dikosongkan.</p>
                        @error('gambar')
                            <span class="text-xs text-red-500 mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Kategori / Label Warta <span
                                class="text-red-500">*</span></label>
                        <input type="text" name="kategori" value="{{ old('kategori') }}" placeholder="Contoh: Program Pemerintah, Bimbingan Teknis"
                            class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                            required>
                        @error('kategori')
                            <span class="text-xs text-red-500 mt-1">{{ $message }}</span>
