...
@section('breadcrumb', 'Master Data / RW / Tambah')

@section('content')
    <div class="max-w-2xl mx-auto space-y-6">
        <div class="flex justify-between items-center">
            <h2 class="text-2xl font-bold text-slate-800">Tambah RW</h2>
        ...
        <x-card>
            <form action="{{ route('superadmin.rw.store') }}" method="POST">
                @csrf
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Nomor RW <span
                                class="text-red-500">*</span></label>
                        <input type="number" name="nomor_rw" value="{{ old('nomor_rw') }}" required min="1"
                            max="999"
                            class="w-full border-slate-300 rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500"
                            placeholder="Misal: 1 atau 001">
                        @error('nomor_rw')
                            <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                        @enderror
                    </div>
