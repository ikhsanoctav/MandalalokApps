...
@section('breadcrumb', 'Master Data / Kategori UMKM')

@section('content')
    <div class="space-y-6" x-data="kategoriIndex()">
        <div class="flex justify-between items-center">
            <h2 class="text-2xl font-bold text-slate-800">Data Kategori UMKM</h2>
                ...
                @forelse($kategoris as $kategori)
                    <div class="p-4 space-y-3">
                        <div class="flex justify-between items-start">
                            <div>
                                <h4 class="font-bold text-slate-800">{{ $kategori->nama_kategori }}</h4>
                                @php
                                    $isMikro = str_contains(strtolower($kategori->nama_kategori), 'mikro');
                                    $isKecil = str_contains(strtolower($kategori->nama_kategori), 'kecil');
                                    $isMenengah = str_contains(strtolower($kategori->nama_kategori), 'menengah');
                                    
                                    $ruleText = '';
                                    if ($isMikro) {
                                        $val = \App\Models\Setting::get('kategori_mikro_max_omset', 10000000);
                                        $ruleText = 'Aturan: ≤ Rp ' . number_format($val, 0, ',', '.');
                                    } elseif ($isKecil) {
                                        $val = \App\Models\Setting::get('kategori_kecil_max_omset', 50000000);
                                        $ruleText = 'Aturan: ≤ Rp ' . number_format($val, 0, ',', '.');
                                    } elseif ($isMenengah) {
                                        $val = \App\Models\Setting::get('kategori_kecil_max_omset', 50000000);
                                        $ruleText = 'Aturan: > Rp ' . number_format($val, 0, ',', '.');
                                ...
                                @endphp
                                @if($ruleText)
                                    <p class="text-xs text-slate-500 font-semibold mt-1">{{ $ruleText }}</p>
                                @endif
                            </div>
                        </div>
                        <div class="flex items-center gap-2 pt-2 border-t border-slate-50">
                            <a href="{{ route('superadmin.kategori.edit', $kategori->id) }}" class="flex-1 text-center py-1.5 text-xs font-medium text-amber-600 bg-amber-50 rounded-lg hover:bg-amber-100 transition-colors">Edit</a>
                            <button @click="deleteData('{{ $kategori->id }}', '{{ e($kategori->nama_kategori) }}', $event)" class="flex-1 text-center py-1.5 text-xs font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-colors">Hapus</button>
                        </div>
                    </div>
                @empty
                    <div class="p-8 text-center text-slate-400">Belum ada data kategori.</div>
                @endforelse
            </div>
                ...
                <table class="min-w-full text-sm text-left border border-slate-200 rounded-md">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th class="py-3 px-4 font-semibold text-slate-600">Nama Kategori</th>
                            <th class="py-3 px-4 font-semibold text-slate-600">Aturan Batas Omset (Bulanan)</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($kategoris as $kategori)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="py-3 px-4 font-medium text-slate-800">{{ $kategori->nama_kategori }}</td>
                                <td class="py-3 px-4 text-slate-700 font-bold">
                                    @php
                                        ...
                                        if ($isMikro) {
                                            $val = \App\Models\Setting::get('kategori_mikro_max_omset', 10000000);
                                            $ruleText = '≤ Rp ' . number_format($val, 0, ',', '.');
                                        } elseif ($isKecil) {
                                            $val = \App\Models\Setting::get('kategori_kecil_max_omset', 50000000);
                                            $ruleText = '≤ Rp ' . number_format($val, 0, ',', '.');
                                        } elseif ($isMenengah) {
                                ...
                                <td class="py-3 px-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <button @click="deleteData('{{ $kategori->id }}', '{{ e($kategori->nama_kategori) }}', $event)"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="py-10 text-center text-slate-400">Belum ada data kategori.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <x-modal name="importModal" maxWidth="lg">
        <form action="{{ route('superadmin.kategori.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 rounded-t-2xl">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        <h3 class="text-lg leading-6 font-bold text-slate-900" id="modal-title">Import Data Kategori</h3>
                        <div class="mt-4 mb-4">
                    <a href="{{ route('superadmin.kategori.import.template') }}"
            ...
            return {
                deleteData(id, name, event) {
                    ...
                    this.$dispatch('open-confirm-modal', {
                        title: 'Hapus Kategori UMKM',
                        message: `Apakah Anda yakin ingin menghapus kategori "${name}"?`,
                        confirmText: 'Ya, Hapus',
                        action: () => {
                            return fetch(`/superadmin/kategori/${id}`, {
                                method: 'DELETE',
                                headers: {
