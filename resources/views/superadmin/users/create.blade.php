...
@section('breadcrumb', 'User Manajemen / Tambah User')
...
@section('content')
    <div class="space-y-6" x-data="{
        ...
        <div
            ...
            <div>
                <h1 class="text-xl font-bold text-slate-800">Tambah Akun Pengguna Baru</h1>
                <p class="text-xs text-slate-700 mt-0.5">Daftarkan user baru ke sistem dan tetapkan hak aksesnya</p>
            </div>
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="email" name="email" required
                                ...
                                placeholder="alamat@domain.com" value="{{ old('email') }}">
                            @error('email')
                                <span class="text-xs text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-2">
                            <label class="text-xs font-semibold text-slate-600 uppercase tracking-wider">NIK (Nomor Induk Kependudukan)
                            ...
                            <input type="text" name="nik"
                                ...
                                placeholder="320xxxxxxxxxxxxx" value="{{ old('nik') }}">
                            @error('nik')
                                <span class="text-xs text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
                    ...
                    <div class="p-6 space-y-6">
                        <div class="space-y-2">
                            ...
                            <select name="role" required x-model="roleVal" @change="roleVal = $event.target.value"
                        ...
                        <div class="space-y-2" x-show="roleVal && roleVal !== 'pelaku_umkm'" x-cloak>
                            ...
                            <input type="text" name="jabatan"
                                class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all outline-none @error('jabatan') border-red-400 @enderror"
                                placeholder="Contoh: Kepala Seksi, Staff Pelaksana" value="{{ old('jabatan') }}">
                            @error('jabatan')
                                <span class="text-xs text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div x-data="{
                            ...
                            get currentRts() {
                                ...
                                const rw = this.currentRws.find(r => r.nomor_rw == this.selectedRw);
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    ...
                                    <select name="id_kelurahan" @change="selectedKel = $event.target.value"
                                        ...
                                        <template x-for="kel in kelurahans" :key="kel.id">
                                            <option :value="String(kel.id)" x-text="kel.nama_kelurahan" :selected="String(kel.id) == String(selectedKel)"></option>
                                        </template>
                                    </select>
                                </div>
                                <div class="space-y-2">
                                    ...
                                    <select name="rw" @change="selectedRw = $event.target.value"
                                        class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all outline-none">
                                        <option value="">-- Semua RW --</option>
                                        <template x-for="rw in currentRws" :key="rw.id">
                                            <option :value="String(rw.nomor_rw)"
                                                x-text="'RW ' + String(rw.nomor_rw).padStart(3, '0')"
                                                :selected="String('{{ old('rw') }}') == String(rw.nomor_rw)"></option>
                                        </template>
                                    </select>
                                </div>
                                <div class="space-y-2">
                                    ...
                                    <select name="rt"
                                        ...
                                        <template x-for="rt in currentRts" :key="rt.id">
                                            <option :value="String(rt.nomor_rt)"
                                                x-text="'RT ' + String(rt.nomor_rt).padStart(3, '0')"
                                                :selected="String('{{ old('rt') }}') == String(rt.nomor_rt)"></option>
                                        </template>
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="bg-slate-50/50 px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-700">Atur kata sandi untuk akun ini</p>
                            </div>
                        </div>
                    </div>
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label class="text-xs font-semibold text-slate-600 uppercase tracking-wider">Password <span class="text-red-500">*</span></label>
                            <div class="relative">
                        ...
                        <div class="space-y-2">
                            <label class="text-xs font-semibold text-slate-600 uppercase tracking-wider">Konfirmasi Password <span class="text-red-500">*</span></label>
                            <div class="relative">
                ...
                <div class="bg-slate-50/50 rounded-2xl border border-slate-100 p-5">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div class="flex items-center gap-3 text-xs text-slate-400">
                            ...
                            <span>Pastikan semua data bertanda <span class="text-red-500">*</span> telah terisi dengan benar.</span>
                        </div>
                        <div class="flex items-center gap-3 w-full sm:w-auto">
                            <button type="reset"
                                class="flex-1 sm:flex-none text-center px-5 py-2.5 rounded-lg border border-slate-200 text-slate-600 font-semibold text-sm hover:bg-slate-100 transition-colors">Reset Form</button>
                            <button type="submit"
