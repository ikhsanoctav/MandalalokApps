<div class="block md:hidden divide-y divide-slate-100">
    @forelse($pengajuans as $index => $pengajuan)
        @php
            ...
            $jenisClass = $jenisBadge[$pengajuan->jenis_pengajuan] ?? 'bg-slate-100 text-slate-700 border-slate-200';
        @endphp
        <article class="p-4 space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3 text-sm">
                <div>
                    <p class="text-xs font-semibold text-slate-400 uppercase">Jenis</p>
                    <span class="inline-flex px-2.5 py-1 rounded-full text-xs font-semibold border {{ $jenisClass }}">
                ...
                <div>
                    ...
                    <p class="font-medium text-slate-700">{{ $pengajuan->tanggal_pengajuan ? $pengajuan->tanggal_pengajuan->format('d M Y') : '-' }}</p>
                </div>
            </div>

            <div>
                <p class="text-xs font-semibold text-slate-400 uppercase">Nominal</p>
            ...
            <div class="grid grid-cols-2 gap-2 pt-1">
                ...
                @if ($pengajuan->status == 'menunggu')
                    <button onclick="updateStatus('{{ $pengajuan->id_pengajuan }}', 'proses', this.dataset.nama)"
                        data-nama="{{ e($pengajuan->umkm->nama_usaha ?? 'UMKM') }}"
                        class="inline-flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg bg-indigo-50 text-indigo-700 text-sm font-semibold">
                        Proses
                    </button>
                @endif
                @if ($pengajuan->status == 'menunggu' || $pengajuan->status == 'proses')
                    <button onclick="updateStatus('{{ $pengajuan->id_pengajuan }}', 'disetujui', this.dataset.nama)"
                        data-nama="{{ e($pengajuan->umkm->nama_usaha ?? 'UMKM') }}"
                    ...
                    <button onclick="updateStatus('{{ $pengajuan->id_pengajuan }}', 'ditolak', this.dataset.nama)"
                        data-nama="{{ e($pengajuan->umkm->nama_usaha ?? 'UMKM') }}"
                        class="inline-flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg bg-rose-50 text-rose-700 text-sm font-semibold">
                        Tolak
                    </button>
                @endif
            </div>
