...
@section('content')
    <div class="space-y-8 pb-12" x-data="{ 
                    ...
                    <div class="text-center sm:text-left flex-grow">
                        ...
                        <div class="mt-4">
                            @if($pemilik)
                                ...
                                @else
                                    <span class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold bg-emerald-100/80 text-emerald-700 border border-emerald-200 shadow-sm backdrop-blur-md">
                                        <i class="mdi mdi-check-decagram text-lg"></i> Akun Terverifikasi Otomatis
                                    </span>
                                @endif
                            @else
        ...
        <div x-show="isEditing" x-cloak class="bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-2xl shadow-indigo-100/50 border border-white overflow-hidden">
            <div class="bg-indigo-600 p-6 sm:px-10 flex items-center justify-between">
                <h3 class="text-xl font-bold text-white flex items-center gap-2">
                    <i class="mdi mdi-account-edit-outline text-2xl text-indigo-200"></i> Form Edit Data Diri
                </h3>
                <span class="px-3 py-1 bg-white/20 rounded-full text-xs font-bold text-white tracking-widest uppercase">Perbarui Data</span>
            </div>

            <form action="{{ route('pelaku.profil.update') }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-10">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="md:col-span-2" x-data="{ showNik: false }">
                        ...
                        <div class="relative flex items-center group">
                            <input :type="showNik ? 'text' : 'password'" value="{{ $user->nik }}" readonly
                                class="w-full bg-slate-50 border-2 border-slate-200 text-slate-500 font-bold text-lg rounded-xl block p-4 pr-32 cursor-not-allowed font-mono tracking-widest transition-colors">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" id="nama_lengkap" name="nama_lengkap"
                            ...
                            class="w-full bg-white border-2 border-slate-200 text-slate-800 font-medium text-base rounded-xl focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 block p-4 transition-all">
                        @error('nama_lengkap')
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="no_hp" name="no_hp" required placeholder="81234567890"
                                ...
                                class="w-full bg-white border-2 border-slate-200 text-slate-800 font-medium text-base rounded-xl focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 block p-4 pl-12 transition-all">
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="alamat" name="alamat" rows="3" required placeholder="Contoh: Jl. Merdeka No. 123..."
                            class="w-full bg-white border-2 border-slate-200 text-slate-800 font-medium text-base rounded-xl focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 block p-4 transition-all resize-none">{{ old('alamat', $pemilik->alamat ?? $user->alamat ?? '') }}</textarea>
                    </div>
                ...
                <div class="mt-10 flex flex-col-reverse sm:flex-row justify-end gap-4 pt-6 border-t border-slate-100">
                    @if($pemilik)
                        <button type="button" @click="isEditing = false"
                            class="w-full sm:w-auto px-8 py-4 bg-white border-2 border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-colors">Batalkan</button>
                    @else
                        <a href="{{ route('pelaku.dashboard') }}"
                            class="w-full sm:w-auto px-8 py-4 bg-white border-2 border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-colors text-center">Kembali ke Dasbor</a>
                    @endif
                    <button type="submit"
                        class="w-full sm:w-auto px-10 py-4 bg-indigo-600 text-white font-bold rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all hover:-translate-y-1 flex justify-center items-center gap-2">
